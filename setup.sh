...
authelia_compose_file="docker-compose.authelia.yml"

compose_file_env="$compose_file"

if [ "$proxy" = "caddy" ]; then
	compose_file_env="${compose_file_env}:$caddy_compose_file"
else
	compose_file_env="${compose_file_env}:$nginx_compose_file"
fi

if [ "$with_authelia" = true ]; then
	compose_file_env="${compose_file_env}:$authelia_compose_file"
fi
...
if [[ "$with_authelia" == true ]]; then
	...
	update_env_vars "AUTHELIA_SCHEMA=authelia" "AUTHELIA_SESSION_SECRET=$(gen_hex 32)" "AUTHELIA_STORAGE_ENCRYPTION_KEY=$(gen_hex 32)" "AUTHELIA_IDENTITY_VALIDATION_RESET_PASSWORD_JWT_SECRET=$(gen_hex 32)"
	...
	update_yaml_file '.services.db.environment.AUTHELIA_SCHEMA = "${AUTHELIA_SCHEMA:?error}" |
